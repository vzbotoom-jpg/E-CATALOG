    ...
    <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-xl font-bold text-black">Edit Portfolio</h1>
            </div>
        </div>

        <form method="POST" action="{{ route('admin.portfolios.update', $portfolio->id) }}" enctype="multipart/form-data" class="p-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" rows="4" 
                          class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:border-black">{{ old('description', $portfolio->description) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="number" name="year" value="{{ old('year', $portfolio->year) }}"
                           class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:border-black">
                </div>
            </div>
            ...
            <div>
                ...
                @if($portfolio->image)
                    <div class="mb-2">
                        <img src="{{ Storage::url($portfolio->image) }}" class="w-20 h-20 object-cover rounded-lg">
            ...
            <div>
                ...
                @if($portfolio->gallery)
                    <div class="flex flex-wrap gap-2 mb-2">
                        @foreach(json_decode($portfolio->gallery, true) as $gallery)
                ...
                <p class="text-xs text-gray-400 mt-1">Upload ulang akan mengganti semua galeri</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="flex items-center gap-4 pt-6">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_featured" value="1" {{ $portfolio->is_featured ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-300 text-yellow-500">
                        <span class="text-sm text-gray-700">Jadikan Unggulan</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ $portfolio->is_active ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-300 text-black">
